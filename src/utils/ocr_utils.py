...
class OCRUtils:
    ...
    def __init__(self,
        ...
        self.ocr = None
        if OCRUtils._global_known_hash_db is not None:
            self.known_hash_db = OCRUtils._global_known_hash_db
        else:
            self.known_hash_db = None

        if OCRUtils._global_char_vector_db is not None:
            self.char_vector_db = OCRUtils._global_char_vector_db
            self.char_vector_labels = OCRUtils._global_char_vector_labels
            self.char_vector_shape = OCRUtils._global_char_vector_shape
        else:
            self.char_vector_db = None
            self.char_vector_labels = None
            self.char_vector_shape = None  # (H, W)

        if OCRUtils._global_char_freq_db is not None:
            self.char_freq_db = OCRUtils._global_char_freq_db
        else:
            self.char_freq_db = None

        if self.use_ocr:
            if OCRUtils._global_ocr is not None:
                self.ocr = OCRUtils._global_ocr
            else:
                import paddle
                ...
                OCRUtils._global_ocr = self.ocr
        else:
            self.ocr = None

        if self.known_hash_db is None:
            state = self.load_known_images(self.known_image_folder, self.known_mapping_json)
            OCRUtils._global_known_hash_db = self.known_hash_db

        if self.char_vector_db is None:
            state = self.load_known_vector_db()
            OCRUtils._global_char_vector_db = self.char_vector_db
            OCRUtils._global_char_vector_labels = self.char_vector_labels
            OCRUtils._global_char_vector_shape = self.char_vector_shape

        if self.use_freq:
            if self.char_freq_db is None:
                state = self.load_char_freq_db()
                OCRUtils._global_char_freq_db = self.char_freq_db
